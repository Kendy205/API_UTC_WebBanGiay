...
namespace WebBanHang.Service.Services
{
    public class CartItemService : ICartItemService
    {
        ...
        {
            var stockVariant = await _unitOfWork.ProductVariant.GetFirstOrDefaultAsync(x => x.VariantId == variantId);
            if (stockVariant == null)
            {
                throw new InvalidOperationException("Không thấy biến thể");
            }

            if (quantity > stockVariant.StockQuantity)
            {
                throw new InvalidOperationException("vượt quá số lượng tồn kho");
            }

            var existingItem = await _unitOfWork.CartItem.GetFirstOrDefaultAsync(x => x.CartId == cartId && x.VariantId == variantId);
            ...
            {
                var variant = await _unitOfWork.ProductVariant.GetFirstOrDefaultAsync(x => x.VariantId == variantId, includeProperties: "Product");
                if (variant == null)
                {
                    throw new InvalidOperationException("Không thấy biến thể");
                }
                ...
                var newItem = new CartItem
                {
                    ...
                    VariantId = variantId,
                    Quantity = quantity,
                    UnitPrice = (decimal)(variant.PriceOverride ?? (variant.Product.SalePrice > 0 ? variant.Product.SalePrice : variant.Product.BasePrice)),
